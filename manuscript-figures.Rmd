...
``` {r}
# 1. change in non-normalized sensitivity as KH: 1 to 100
compzoom_dR0dk.df_1 <- dR0dk.df %>%
  filter(sigmaH == 10, focal_var != "Overall") %>% 
  select(system_ID, Temperature, sigmaH, KH, focal_var, system_label, median) %>%
  pivot_wider(names_from = KH, values_from = median) %>%
  mutate(abs_comp = `1` - `100`)

# 2. Absolute change in non-normalized sensitivity as KH: 1 to 100
compzoom_dR0dk.df_2 <- dR0dk.df %>%
  filter(sigmaH == 10, focal_var != "Overall") %>% 
  select(system_ID, Temperature, sigmaH, KH, focal_var, system_label, median) %>%
  pivot_wider(names_from = KH, values_from = median) %>%
  mutate(abs_comp = abs(`1` - `100`))

# 3. Proportional change in non-normalized sensitivity as KH: 1 to 100
compzoom_dR0dk.df_3 <- dR0dk.df %>%
  filter(sigmaH == 10, focal_var != "Overall") %>% 
  select(system_ID, Temperature, sigmaH, KH, focal_var, system_label, median) %>%
  pivot_wider(names_from = KH, values_from = median) %>%
  mutate(abs_comp = (`1` - `100`)/(`1`+1))

# 4. Proportional change in non-normalized sensitivity as KH: 1 to 100
compzoom_dR0dk.df_4 <- dR0dk.df %>%
  filter(sigmaH == 10, focal_var != "Overall") %>% 
  select(system_ID, Temperature, sigmaH, KH, focal_var, system_label, median) %>%
  pivot_wider(names_from = KH, values_from = median) %>%
  mutate(temp_comp = `1` - `100`) %>% 
  group_by(system_ID, Temperature) %>% 
  mutate(total_comp = sum(temp_comp),
         abs_comp = temp_comp / sum(temp_comp))
