...
MosqThermTraits_df <- median.Vec %>%
  ...
  mutate(variable = factor(variable,
                           ...
                           labels = c(
                             ...
                             TeX("(d) Immature mosquito development rate: $\\rho_L$ (day$^{-1}$)"),
                             TeX("(e) Probability of immature mosquito survival: $\\delta_L$"),
...
```{r fig-S8_2_dR0dvar_uncertainty, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:dR0dvar_unc} The local sensitivity of $\\mathcal{R}_0$ to mosquito traits across temperatures for two values of $\\sigma_H$ and $K_H$. Generally, $\\mathcal{R}_0$ is most sensitive to biting frequency, the probability of surviving the extrinsic incubation period, and vector competence. Shown here are the median values of the derivative of $\\mathcal{R}_0$ with respect to each focal mosquito trait across the mosquito trait TPC hyperparameter posterior distributions. Sensitivity values are shown on a logarithmic scale with solid lines indicating positive values and dotted lines indicating negative. Temperatures are restricted to those at which the median of $\\mathcal{R}_0$ is positive for the given system. '}
KH_vals <- c(1, 100)
...
dR0dk.df <- data.dR0dT %>%
  ...
  # Restrict to relevant temperatures (V0 and R0 > 0)
...
# # R0 derivative dataframe
# R0deriv_df <- data.R0 %>% 
#   group_by(system_ID, Model, sigmaH, KH) %>% 
#   arrange(system_ID, Model, sigmaH, KH, Temperature) %>%
#   mutate(deriv = (mean - lag(mean)) / (Temperature - lag(Temperature)))

# test_plot <- R0deriv_df %>% 
#   filter(KH %in% c(1), sigmaH %in% c(100, Inf), median > 0) %>% 
#   ggplot(aes(x = Temperature, y = deriv)) +
#   geom_line() +
...
plot.dR0dk <- dR0dk.df %>%
  ...
  ggplot(aes(x = Temperature, y = norm_median, color = focal_var, 
  ...
  scale_y_continuous(
    name = unname(TeX("Normalized local sensitivity of $\\,R_0$ to temperature")),
    # limits = c(-1, 1),
    # limits = 10^c(-4, 4),
    # breaks = 10^(seq(-4, 4)),
    # labels = TeX(c("$10^{-4}$", "$10^{-3}$", "$10^{-2}$", "$10^{-1}$", "$10^0$", "$10^1$", "$10^2$", "$10^3$", "$10^4$")),
    # trans = "log10",
    expand = c(0, 0)
  ) +
  # scale_linetype_manual(
  #   name = "Sign of sensitivity:",
  #   values = c(1, 3),
  #   breaks = c(1, -1),
...
```{r fig-S8_3_dR0dvar_comparison, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:dR0dvar_comp} Comparison of the local sensitivities of $\\mathcal{R}_0$ to mosquito traits across temperatures for two values of $\\sigma_H$. Generally, $\\mathcal{R}_0$ is most sensitive to biting frequency, the probability of surviving the extrinsic incubation period, and vector competence. Shown here are the median values of the derivative of $\\mathcal{R}_0$ with respect to each focal mosquito trait across the mosquito trait TPC hyperparameter posterior distributions. Sensitivity values are shown on a logarithmic scale with solid lines indicating positive values and dotted lines indicating negative. Temperatures are restricted to those at which the median of $\\mathcal{R}_0$ is positive for the given system. '}
...
# filter(focal_var %in% c("Parasite development rate", "Biting frequency"))
...
# mutate(sens_comp = ifelse(`10` == 0 & `1` == 0,
#                           0,
#                           (`10` - `1`) / ((`10` + `1`)/2)))
# mutate(sens_comp = (`10` - `1`) / `1`)
...
```{r fig-S8_4_dR0dvar_compzoom, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:dR0dvar_comp} Comparison of the local sensitivities of $\\mathcal{R}_0$ to mosquito traits across temperatures for two values of $\\sigma_H$. Generally, $\\mathcal{R}_0$ is most sensitive to biting frequency, the probability of surviving the extrinsic incubation period, and vector competence. Shown here are the median values of the derivative of $\\mathcal{R}_0$ with respect to each focal mosquito trait across the mosquito trait TPC hyperparameter posterior distributions. Sensitivity values are shown on a logarithmic scale with solid lines indicating positive values and dotted lines indicating negative. Temperatures are restricted to those at which the median of $\\mathcal{R}_0$ is positive for the given system. '}

compzoom_dR0dk.df <- comp_dR0dk.df %>%
  filter(sigmaH == 10) %>% 
  select(system_ID, Temperature, sigmaH, KH, focal_var, system_label, prop_sens) %>%
  pivot_wider(names_from = KH, values_from = prop_sens) %>%
  mutate(abs_comp = abs(`1`) - abs(`100`))

plot.compzoom_dR0dk <- compzoom_dR0dk.df %>% 
  # filter(focal_var %in% c("Parasite development rate", "Adult mortality rate", "Biting frequency")) %>%
  # Make a stacked plot, colored by parameter
...
```{r fig-S8_R0_uncertainty, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:R0_unc}  The uncertainty of $\\mathcal{R}_0$ to mosquito traits across temperatures as measured by the change in the 95\\% highest posterior density interval width for two values of $\\sigma_H$ when vertebrate host population density is set to one individual per hectare. Temperatures are restricted to those at which $\\mathcal{R}_0$ is positive for the given system. At the cool end of the thermal niche, variation in $\\mathcal{R}_0$ is mostly driven by the probability of surviving the extrinsic incubation period, except in the case of (d) \\textit{An. gambiae} and \\textit{P. falciparum}. As temperature is increased, the influence of adult mosquito lifespan also increases to eventually become the most influential parameter for all systems except (e) \\textit{Cx. quinquefasciatus} and WNV.'}
