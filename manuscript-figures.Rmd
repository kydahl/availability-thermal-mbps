...
# Run 'code/vector-traits.R' and 'get-outputs.R' first to produce these data

# Vector trait thermal performance curve data
data.Vec <- read_rds("results/VecTPC_vals.rds") %>%
  pathosystem_labeller()

# R0 data: thermal performance curves and heatmap
...
```{r fig-4_R0_TPCs, dev = c('pdf','svg', 'png'),  message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:R0_plots} $\\mathcal{R}_0$ as a function of temperature has the usual properties of a thermal performance curve (TPC): it is positive on a single bounded interval and has a unique maximum value. The shape of the $\\mathcal{R}_0$ TPCdepends on vertebrate host availability and the mosquito-parasite system. The shape of the thermal response of $\\mathcal{R}_0$ derived from the Ross-Macdonald model (dotted black curves) is independent of vertebrate host population density while those derived from the Chitnis dynamic model are dependent on vertebrate host population density (solid lines in color). Shown here are the median values of $\\mathcal{R}_0$ with respect to the mosquito trait TPC posterior distributions. $\\mathcal{R}_0$ is normalized by dividing by its maximum value in each system, for the given value of biting tolerance and vertebrate host population density. Vertebrate host population density is varied on a log$_{10}$ scale.'}
...
# plots = one for each mosquit0o+pathogen combination, "system_ID"
...
# Value of sigmaH to replace 'Inf' with for visual plotting
infinite_skew <- 10^2.35
# Used to create a visual gap dividing finite and infinite sigmaH
...
# Define visual tick breaks and labels for sigmaH (biting tolerance)
...
Toptalt_plots <- Toptalt_df %>%
  ...
  # R0 = 1 contour: uses where CT_width = 0 (i.e. where R0<1 at any temperature)
  geom_contour(
...
```{r fig-7_explain_sensitivity, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:dR0dvar_comp} The change in the contributions of the temperature-dependent mosquito traits to the overall temperature-sensitivity of $\\mathcal{R}_0$ as vertebrate host population density is increased from $1$ to $100$ individuals per hectare. There is a fairly consistent pattern in the change in importance for four out of the five systems.But, unlike the others, the \\textit{Anopheles gambiae} and \\textit{Plasmodium falciparum} system displays large changes in the contributions of biting frequency and parasite development rate at low temperatures and relatively small changes for adult mortality rate at warmer temperatures. This indicates that, through the incorporation of limited biting tolerance, a change in vertebrate host availability shifts the importance of certain temperature-dependent mosquito traits in determining the temperature-dependence of $\\mathcal{R}_0$. Shown here are the median values of $\\bar{\\mathcal{S}_p}(T, 100, 10) - \\bar{\\mathcal{S}_p}(T, 1, 10)$ as described by equation (2.4).'}

# Set values for vertebrate host population density and biting tolerance
KH_vals <- c(1, 100)
sigmaH_val <- 10

# Calculate temperature limits for each system
temp_lims <- select(data.R0, system_ID, system_label, Temperature, Model, sigmaH, KH, median_R0 = median) %>%
  filter(KH %in% KH_vals & sigmaH == sigmaH_val) %>%
...
# Calculate 'proportional contribution to sensitivity' for plotting
dR0dT.df <- data.dR0dvar %>%
  # Filter to relevant values of KH and sigmaH
  filter(KH %in% KH_vals & sigmaH == sigmaH_val) %>% 
  # Restrict to temperatures where R0 > 0
  right_join(temp_lims) %>%
  group_by(system_ID) %>%
  filter(Temperature > min_Temp, Temperature < max_Temp) %>%
  filter(!is.na(median)) %>%
  ungroup() %>%
  # Remove 'Overall' sensitivity: this will be used for comparison
  filter(focal_var != "Overall") %>%
  group_by(system_ID, Temperature, sigmaH, KH) %>%
  mutate(
    # Absolute value of R0 sensitivities
    abs_median = abs(median),
    # Total absolute sensitivity
    total_sens = sum(abs_median),
    # Proportion of sensitivity due to particular trait
    prop_sens = abs_median / total_sens
  ) %>%
  arrange(system_label, Temperature, KH, sigmaH, focal_var) %>% 
  # Calculate change in contribution as host pop. density increases from 1 to 100
...
# Plot figure
plot.compzoom_dR0dk <- dR0dT.df %>%
  ggplot(aes(x = Temperature, y = abs_comp, fill = focal_var, color = focal_var, )) +
  # Plot change in sensitivity contributions
  geom_line(lwd = 1) +
  # Horizontal line at zero, to emphasize change in sign
  geom_hline(yintercept = 0, lwd = 0.25) +
  # Color and fill: by focal trait
  scale_color_manual(
    name = "Focal trait:",
...
```{r fig-S1_MosquitoDensity, dev = c('pdf','svg', 'png'),  out.width='100%', fig.height=6, fig.width=9, fig.align='center', fig.cap='\\label{fig:MosquitoDensity} Equilibrium mosquito population density, $V^*(T)$, as a function of temperature. Shown here are the median values of $V^*(T)$ with respect to the mosquito trait TPC hyperparameter posterior distributions. Because infection was not assumed to affect life history traits, the \\textit{Aedes aegypti} and DENV and \\textit{Ae. aegypti} and ZIKV curves overlap. As shown in Equation (2.1), $V^*(T)$ is a function of the immature mosquito development rate, probability of immature mosquito survival to adulthood, eggs per female per day, the adult mortality rate, and the carrying capacity for larval mosquitoes. This carrying capacity was assumed to be temperature-independent and set to 300, which resulted in a plausible range of mosquito population densities for the five systems. '}
...
```{r fig-S2_R0_heatmap_plots_all, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=11, fig.align='center', fig.cap='\\label{fig:R0_heatmap_plots_all} $\\mathcal{R}_0$ as a function of temperature and biting tolerance  ($\\sigma_H$) across three levels of vertebrate host population density: $0.1$, $1$, and $10$ vertebrate hosts per hectare. Shown here are the median values of $\\mathcal{R}_0$ with respect to mosquito trait TPC hyperparameter posterior distributions. Vertebrate host biting tolerance is varied on a $\\log_{10}$ scale with a break to indicate values of $\\mathcal{R}_0$ when biting tolerance is unlimited.'}
...
```{r fig-S6_CTmax, dev = c('pdf','svg', 'png'), dev="cairo_pdf", message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:CTmax} The critical thermal maximum for parasite population persistence as a function of vertebrate host population density and biting tolerance. Grey areas indicate combinations of vertebrate host population density and biting tolerance in which the parasite population is expected to not persist at any temperature. In the region where vertebrate host population density and biting tolerance are both less than 10, critical thermal minima and maxima are very similar (see Figure 6 in the main text). Shown here are the median values of the critical thermal maximum with respect to mosquito trait TPC hyperparameter posterior distributions. Both vertebrate host population density and biting tolerance are varied on a $\\log_{10}$ scale with a break in biting tolerance values to indicate when biting tolerance is unlimited.'}
...
# Numerical estimates of the difference between Topt and Topt^RM
...
# Calculating Topt_0: Chitnis case
Topt0_df <- median.Vec %>%
  filter(V0 > 1) %>%
  mutate(muV = 1 / (lf + eps)) %>%
  mutate(P_EIP = exp(-muV / etaV)) %>%
...
# Calculating Topt_0: Ross-Macdonald case
...
# Note: the following text was added via Adobe Illustrator for cases where R0 never exceeds 1:
# "R_0 < 1 at all temperatures"
```

```{r fig-S9_R0_sensitivity, dev = c('pdf','svg', 'png'), message=FALSE, out.width='100%', fig.width=9, fig.height=6, fig.align='center', fig.cap='\\label{fig:dR0dvar_unc} The local sensitivity of $\\mathcal{R}_0$ to mosquito traits across temperatures for two values of biting tolerance ($\\sigma_H$) and vertebrate host population density ($K_H$). Overall, the sensitivity of $\\mathcal{R}_0$ to temperature decreases at higher temperatures. At high temperatures, adult mortality rate is the primary contributor to the temperature sensitivity of $\\mathcal{R}_0$ across all systems and all values of $\\sigma_H$ and $K_H$. At lower temperatures, the most important trait varies between the adult mortality rate, biting frequency, parasite development rate, or vector competence, depending on the system in question and values of the vertebrate host traits. The values shown here are the median values taken across the mosquito trait TPC hyperparameter posterior distributions of the contributions of each trait to the local sensitivity of derivative of $\\mathcal{R}_0$ to temperature: $\\frac{1}{\\mathcal{R}_0}\\frac{\\partial\\mathcal{R}_0}{\\partial p}\\frac{dp}{dT}$ where $p$ is a focal trait and $T$ is temperature. Values were scaled to span from -1 to 1 to enable comparisons across systems. Temperatures are restricted to those at which $\\mathcal{R}_0$ is positive for the given system. '}

# Set population density values of interest
KH_vals <- c(1, 100)

# Get temperature limits for each system where R0 > 0
temp_lims <- select(data.R0, system_ID, system_label, Temperature, Model, sigmaH, KH, median_R0 = median) %>%
...
dR0dT.df <- data.dR0dvar %>%
  # Filter to KH = 1, 100 and sigmaH = 10, Inf
...
plot.dR0dk <- dR0dT.df %>%
  ...
  ggplot(aes(
    x = Temperature, y = median, color = focal_var,
  )) +
  # lines: values of R0 sensitivities
  geom_line(lwd = 0.5) +
  # add a line at y = 0 to emphasize sign changes
  geom_hline(yintercept = 0, lwd = 0.25) +
  # color: focal trait
  scale_color_manual(
    name = "Focal trait:",
  ...
  # Restrict y-axis to -1 to 1
  coord_capped_cart(ylim = c(-1, 1)) +
  # Rows = combinations of KH and sigmaH
  # Columns = systems
  facet_grid(
    rows = vars(KH, sigmaH),
    cols = vars(system_label),
...
plot.CTmin.rel.sens <- CTmin_df %>%
  ggplot(aes(x = KH, y = sens_rel, fill = focal_var)) +
  # Area plot of relative sensitivity/uncertainty
  geom_area() +
  # fill: focal trait
  scale_fill_manual(
    name = "Focal trait:",
