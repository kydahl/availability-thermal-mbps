...
# Initialize a data frame with correct column names and types
init.df <- tibble(
  system_ID = character(), Temperature = double(),
...
data.R0 <- as.data.frame(data.Host) %>%
  # Filter to values of sigmaH and KH used for plotting
  filter(sigmaH %in% c(
    10^seq(-1, 2), Inf,
...
data.Topt <- as.data.frame(data.Host) %>%
  # Filter to sigmaH and KH values used for visualizations
  filter(sigmaH %in% c(
    10^seq(-1, 2), Inf,
    unique(sigmaH)[seq(1, length(unique(sigmaH)), length.out = 51)]
  )) %>%
...
# Set up iteration grid
iter_grid <- expand_grid(
  system_ID = unique(data.Vec$system_ID),
  tibble(KH = KH_slices)
) %>%
  expand_grid(sigmaH = sigmaH_slices)
